...
% Author: Philipp Ehses (philipp.ehses@tuebingen.mpg.de), Mar/11/2014
    ...
    for b=1:nbuffers
        ...
        if b==1
            prot.Config = parse_buffer(buffer,bufname);
        elseif b==2
            prot.Dicom = parse_buffer(buffer,bufname);
        elseif b==3
            prot.Meas = parse_buffer(buffer,bufname);
        elseif b==4
            prot.MeasYaps = parse_buffer(buffer,bufname);
        elseif b==5
            prot.Phoenix = parse_buffer(buffer,bufname);
        elseif b==6
            prot.Spice = parse_buffer(buffer,bufname);
        end
    end
